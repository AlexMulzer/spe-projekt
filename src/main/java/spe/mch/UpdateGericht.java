 ...
 * @author Alex
 */
@WebServlet(name="UpdateGericht", urlPatterns={"/UpdateGericht"})
public class UpdateGericht extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String sql = "update gerichte set name = ?, preis = ?, beschreibung = ?, kategorie = ? where nummer = ?";
        ConnectionPool dbPool = (ConnectionPool)getServletContext().getAttribute("dbPool");
        Connection conn = dbPool.getConnection();

        try{
            PreparedStatement pstm = conn.prepareStatement(sql);
            pstm.setInt(5, nummer);
